...
const Checkout = ({ cart }) => {
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < cart.length; index++) {
      // const element = cart[index][1];
      subTotal = subTotal + cart[index][2]
      qty = qty + cart[index][1]
      product.push(cart[index][0])
    }
    setproduct(product)
      ...
      <section className="text-gray-600 body-font relative" style={{ userSelect: 'auto' }}>
        <div className="container mx-auto px-5 py-24 min-h-screen" style={{ userSelect: 'auto' }}>
          <div className="flex flex-col  w-full mb-12" style={{ userSelect: 'auto' }}>
            <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900" style={{ userSelect: 'auto' }}>Checkout</h1>
            <h2 className='font-bold text-black  text-xl'>cart</h2>
            <div>{cart.length ?' Your Cart detial as follows:-': "your cart is empty!"}</div>
            <ul>
              {cart.map((item) => {
                return <li className='list-decimal mx-4'>
                  {item[0]} price is {item[2]}
            ...
            <p>
              <b >Total Quantity is:</b> {qty}
            </p>
            <p> <b>Total price is  :</b> {subTotal}</p>
          </div>
          <div className="lg:w-full md:w-full " style={{ userSelect: 'auto' }}>
            <div className="flex flex-wrap -m-2" style={{ userSelect: 'auto' }}>
              ...
              <div className="p-2 w-1/2" style={{ userSelect: 'auto' }}>
                  <div className="relative" style={{ userSelect: 'auto' }}>
                    <label htmlFor="email" className="leading-7 text-sm text-gray-600" style={{ userSelect: 'auto' }}>Phone</label>
                    <input  onChange={handleChange} type="phone" id="phone" name="phone" value ={form.phone} className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" style={{ userSelect: 'auto' }} />
                  </div>
                </div>
              <div className="p-2 w-full" style={{ userSelect: 'auto' }}>
                <div className="relative" style={{ userSelect: 'auto' }}>
                  <label htmlFor="address" className="leading-7 text-sm text-gray-600" style={{ userSelect: 'auto' }}>Address</label>
